...
having it overwritten. These can of course also have slaves.

==== Readable master database

If you want, you can schedule your master database to be read from alongside the
slaves, by including 'readable: true' in your database.yml config for it.

==== Slaves are no longer required

Including the gem and calling MultiDb::ConnectionProxy.setup! no longer throws
an execption if there are not slave databases defined. This can be especially
useful in conjunction with the first item in this changelist.
